...
test('Search page displays information for adult criminal case', async ({ page }) => {
  ...
  await expect(page.locator('#main-content')).toContainText('Ticket Date:');
  await expect(page.locator('#main-content')).toContainText('05/19/2025');
});
...
test('Search page displays information for juvenile case', async ({ page }) => {
  ...
  await expect(page.locator('#main-content')).toContainText('Case Number:');
  await expect(page.locator('#main-content')).toContainText('25-JV-25-4');
  await expect(page.locator('#main-content')).toMatchAriaSnapshot(`
    ...
    - text: "/Ticket Date: \\\\d+\\\\/\\\\d+\\\\/\\\\d+ Total Owned: \\\\$\\\\d+\\\\.\\\\d+/"
    `);
  });
...
test('Search page displays information for adult civil case', async ({ page }) => {
  ...
  await expect(page.locator('#main-content')).toMatchAriaSnapshot(`- text: "/Case Number: \\\\d+-CV-\\\\d+-\\\\d+ Party Name: testing, defendant Party Connection: Defendant Case Type: Property Damage Case Location: Scott Case Status: Open Date Filed: \\\\d+\\\\/\\\\d+\\\\/\\\\d+ Total Owned: \\\\$\\\\d+\\\\.\\\\d+/"`);
});