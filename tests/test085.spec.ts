...
test('Payment Plan Payment messaging', async ({ page }) => {
  ...
  await expect(page.locator('#payment-plan-heading')).toContainText('Make Payment Plan Payment');
  ...
  await expect(page.getByLabel('Make Payment Plan Payment')).toMatchAriaSnapshot(`
    - paragraph:
      ...
      - text: /\\d+, \\d+/
    `);
  await expect(page.getByLabel('Make Payment Plan Payment')).toMatchAriaSnapshot(`- paragraph: /Your next installment amount is \\$\\d+\\.\\d+\\. You can pay this installment amount plus a convenience fee of \\$\\d+\\.\\d+\\. You can pay a different amount up to the remaining balance of \\$\\d+\\.\\d+ plus the convenience fee\\./`);
